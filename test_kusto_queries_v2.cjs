        ...
        return new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (this.pendingRequests.has(id)) {
                    ...
                    reject(new Error(`Request timeout for ${method} after 60s`));
                }
            }, 60000);
        });
    }
...
async function runTests() {
    ...
    try {
        ...
        console.log('='.repeat(80));
        console.log('\n1. FUNCTIONALITY:');
        console.log(successfulTests === totalTests ? '   ✓ All tests passed - tool is stable' : `   ⚠ ${failedTests} tests failed - reliability concerns`);
        ...
        console.log('   - Add query complexity indicators');
        console.log('   - Include common use cases for each query');
        
        console.log('\n4. FINAL VERDICT:');
        if (successfulTests >= totalTests * 0.8) {
            console.log('   ✓ RECOMMENDED FOR USE - Tool demonstrates good stability');
        } else {
            console.log('   ⚠ USE WITH CAUTION - Reliability issues detected');
        }
