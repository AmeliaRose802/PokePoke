...
class SDKWatchdog:
    """Monitors SDK session health including tool timeouts, inactivity, and process liveness."""
    ...
    @staticmethod
    def _check_client_state(client: Any, done: asyncio.Event) -> None:
        """Check if client is disconnected and set done event if so."""
        try:
    ...
    async def _check_process_liveness(
        ...
        Returns (should_abort, reason, consecutive_ping_failures).
        """
    ...
    def _get_effective_activity_time(stats: SessionStats) -> tuple[float, bool, float | None]:
        ...
        return effective_last_activity, has_active_children, child_activity_time
    ...
    async def _check_inactivity(
        ...
        Returns True if inactivity was detected and session aborted.
        """
        ...
        effective_last_activity, has_active_children, _ = SDKWatchdog._get_effective_activity_time(stats)
    ...
    async def await_completion(  # noqa: C901
        ...
        while not done.is_set():
            ...
            result = await SDKWatchdog.check_tool_watchdog(session, stats, tool_call_timeout, handler)
