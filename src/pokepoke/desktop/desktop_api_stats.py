...
def get_cached_leaderboard(self: DesktopAPI) -> dict[str, Any]:
    ...
    with self._lock:
        ...
        if now - self._leaderboard_cache_time > 5.0:
            from pokepoke.models.model_stats_store import get_model_summary
...
def get_model_leaderboard(self: DesktopAPI, repo_name: str = "") -> dict[str, Any]:
    ...
    """
    from pokepoke.models.model_stats_store import get_model_summary, get_model_summary_by_repo
    if repo_name:
...
def get_model_history(self: DesktopAPI, limit: int = 200, repo_name: str = "") -> list[dict[str, Any]]:
    ...
    from pokepoke.models.model_history import load_model_history_entries
...
def get_lock_contention_stats(self: DesktopAPI) -> dict[str, Any]:
    ...
    from pokepoke.worktrees.lock_contention import get_lock_contention_stats as _get
    return _get()
