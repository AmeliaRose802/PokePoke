...
This directory contains the main TypeScript source code for the ICM Queue MCP server.
...
├── kusto-client.ts       # Kusto database client wrapper
├── config.ts             # Configuration and connection strings
...
### Kusto Service Pattern

```typescript
export class KustoService {
  private clients = new Map<string, KustoClient>();

  constructor() {
    // Load connection strings from config
  ...
  private getClient(cluster: string): KustoClient {
    if (!this.clients.has(cluster)) {
      const connectionString = this.getConnectionString(cluster);
...
- [ ] No hardcoded cluster/database names (use config)
- [ ] Logging for debugging Kusto queries
- [ ] Performance considerations (parallel queries, result size)
