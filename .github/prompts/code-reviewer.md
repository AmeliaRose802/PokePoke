...
5. File issues in beads

   For each significant issue you find, create a beads issue using `bd create`:
   
   - **Correctness issues**: Priority 0 or 1, type `bug`
   - **Maintainability issues**: Priority 1 or 2, type `task` or `refactor`
   - **Style issues**: Priority 3, type `chore`
   
   Use descriptive titles and include the affected file/function in the description. Add appropriate labels like `code-quality`, `tests`, `refactoring`, etc.
   
   Example:
   ```bash
   bd create "Fix error handling in process_item function" -t bug -p 1 -d "The process_item function in orchestrator.py doesn't handle ValueError exceptions. This could cause crashes if invalid input is provided." --json
   bd label add <id> orchestrator code-quality --json
   ```
   
   Group related issues under a parent issue if there are many similar problems.

6. Prepare a summary report

   Output a brief markdown summary for the terminal. Include:
   - Total issues found by category (correctness, maintainability, style)
   - Count of beads issues created
   - Top 3 most critical issues
   
   Keep it concise - the details are now in beads issues.