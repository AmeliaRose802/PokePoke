...
export function useCopyCompletedItems() {
  ...
  const copyCompletedItems = async (
    ...
    try {
      ...
      const formattedItems = completedItems.map((item, index) => {
        const itemStats = getItemStats(item.id, modelHistory);
        const gateStatus = itemStats?.gate_passed === true ? 'Passed gate' 
                         : itemStats?.gate_passed === false ? 'Failed gate' 
                         : 'Pending';
