...
describe("AgentsPanel", () => {
  ...
  describe("retry attempt cards", () => {
    ...
    it("omits pause/resume controls for historical attempts", () => {
      ...
      render(<AgentsPanel agents={[history]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
    ...
    it("calls onSelectAgent with the card identifier", () => {
      ...
      render(<AgentsPanel agents={[agent]} onSelectAgent={onSelect} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
  ...
  it("collapses completed agents by default and expands on click", () => {
    ...
    render(<AgentsPanel agents={[running, done]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
  ...
  it("collapses active agents on click and expands again", () => {
    ...
    render(<AgentsPanel agents={[running]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
  ...
  it("calls onPauseAgent when pause button is clicked", () => {
    const onPause = vi.fn();
    const agent = mkAgent();
    render(<AgentsPanel agents={[agent]} onPauseAgent={onPause} onResumeAgent={vi.fn()} />);
  ...
  it("calls onResumeAgent when resume button is clicked", () => {
    ...
    const agent = mkAgent({ paused: true });
    render(<AgentsPanel agents={[agent]} onPauseAgent={vi.fn()} onResumeAgent={onResume} />);
  ...
  it("applies paused CSS class to paused agent card", () => {
    const agent = mkAgent({ paused: true });
    const { container } = render(<AgentsPanel agents={[agent]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
  ...
  it("pause click does not trigger card selection", () => {
    ...
    const onPause = vi.fn();
    const agent = mkAgent();
    render(<AgentsPanel agents={[agent]} onSelectAgent={onSelect} onPauseAgent={onPause} onResumeAgent={vi.fn()} />);
    fireEvent.click(screen.getByTitle("Pause agent"));
  ...
  describe("session grouping", () => {
    ...
    it("does not show session headers for single session", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[agent]} currentSessionId="1000.0" onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
    ...
    it("shows agent count in collapsed session header", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={agents} currentSessionId="2000.0" onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );

      const collapsedHeader = container.querySelector('.session-group-header[aria-expanded="false"]');
  ...
  describe("maintenance sub-agent nesting", () => {
    it("renders maintenance sub-agent nested under parent agent", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[janitorAgent, mergeConflictAgent]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
    ...
    it("renders multiple maintenance sub-agents under same parent", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[janitorAgent, subAgent1, subAgent2]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
    ...
    it("does not nest agent when parent is not in agent list", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[orphanAgent]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
  ...
  describe("retry agent nesting", () => {
    it("should nest retry agents under the main agent card", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[mainAgent, retryAgent]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
    ...
    it("should handle multiple retry attempts nested under main agent", () => {
      ...
      const { container } = render(
        <AgentsPanel agents={[mainAgent, retryAgent1, retryAgent2]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />,
      );
    ...
    it('shows "Attempt N" labels for agents in a retry cycle', () => {
      ...
      render(<AgentsPanel agents={[mainAgent, retryAgent]} onPauseAgent={vi.fn()} onResumeAgent={vi.fn()} />);
