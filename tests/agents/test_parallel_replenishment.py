"""Tests for parallel replenishment and circuit breaker behavior.

This module contains regression tests for:
- Batch replenishment (PokePoke-8o4o): filling all empty slots after completions
- Crashed worker handling: exception-crashed workers remain eligible for retry
- Circuit breaker: stops dispatch after consecutive failure rounds
- Single-shot vs continuous replenishment differences
"""
...
class TestParallelReplenishmentBug:
    ...
    @patch("pokepoke.agents.parallel.process_work_item")
    @patch("pokepoke.agents.parallel._get_dynamic_max_agents", return_value=2)
    def test_circuit_breaker_resets_on_success(  # noqa: PLR0913
        self, mock_dyn_max, mock_pwi, mock_collect, mock_sel, mock_ready,
        mock_repo, mock_shut, mock_stop, mock_set_exec, mock_ui, mock_sleep, mock_claimable,
    ) -> None:
