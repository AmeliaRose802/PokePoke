...
from pokepoke.workflow import _setup_worktree, process_work_item
...
def _mock_workflow_deps(monkeypatch):
    ...
    monkeypatch.setattr("pokepoke.workflow.get_config", lambda: mock_cfg)
    monkeypatch.setattr("pokepoke.workflow.terminal_ui", MagicMock())
    monkeypatch.setattr("pokepoke.workflow.register_agent", lambda: None)
    monkeypatch.setattr("pokepoke.workflow.unregister_agent", lambda: None)
    monkeypatch.setattr("pokepoke.workflow.is_shutting_down", lambda: False)
    monkeypatch.setattr("pokepoke.workflow.set_current_work_item_id", lambda x: None)
    monkeypatch.setattr("pokepoke.workflow.set_current_repo_name", lambda x: None)
    monkeypatch.setattr("pokepoke.workflow.get_agent_name", lambda default="agent": "test-agent")
    monkeypatch.setattr("pokepoke.workflow.select_model_for_item", lambda item: "test-model")
    monkeypatch.setattr("pokepoke.workflow.get_assignment_for_item", lambda item: ("test-model", "beads-item"))
    monkeypatch.setattr("pokepoke.workflow.build_prompt_from_work_item", lambda *a, **kw: "test prompt")
    monkeypatch.setattr("pokepoke.workflow.build_resume_prompt", lambda *a, **kw: "resume prompt")
    monkeypatch.setattr("pokepoke.workflow.assign_and_sync_item", lambda *a, **kw: True)
    monkeypatch.setattr("pokepoke.workflow.add_comment", lambda *a, **kw: True)
    monkeypatch.setattr("pokepoke.workflow.cleanup_worktree", lambda *a, **kw: None)

    # Lazy imports mocked at source module
    monkeypatch.setattr("pokepoke.beads.increment_total_attempts", lambda *a: True)
    monkeypatch.setattr("pokepoke.reconciliation.is_beads_item_closed", lambda *a: False)
    monkeypatch.setattr("pokepoke.git_operations.build_handoff_context", lambda **kw: "handoff ctx")
    monkeypatch.setattr("pokepoke.metrics_context.agent_type_context", lambda *a: nullcontext())

    # Mock WorkItemSession to avoid real file operations
    mock_session = MagicMock()
    monkeypatch.setattr("pokepoke.workflow.WorkItemSession", lambda **kw: mock_session)
...
class TestSetupWorktree:
    def test_success_returns_path(self):
        ...
        with patch("pokepoke.workflow.create_worktree", return_value=wt):
            result = _setup_worktree(_item())
        assert result == wt

    def test_failure_returns_none(self):
        with patch("pokepoke.workflow.create_worktree", side_effect=Exception("git error")):
            result = _setup_worktree(_item())
    ...
    def test_failure_logs_to_run_logger(self):
        rl = MagicMock()
        il = MagicMock()
        with patch("pokepoke.workflow.create_worktree", side_effect=RuntimeError("lock")):
    ...
    def test_custom_lock_timeout_forwarded(self):
        with patch("pokepoke.workflow.create_worktree", return_value=Path("/w")) as mock_cw:
...
class TestProcessWorkItemClaimFailure:
    ...
    def test_claim_failure_returns_failed_result(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.assign_and_sync_item", lambda *a, **kw: False)
        result = process_work_item(_item(), interactive=False)
        assert result.success is False

    def test_claim_failure_skips_worktree(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.assign_and_sync_item", lambda *a, **kw: False)
        with patch("pokepoke.workflow._setup_worktree") as mock_sw:
            result = process_work_item(_item(), interactive=False)
...
class TestProcessWorkItemWorktreeFailure:
    ...
    def test_worktree_failure(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.create_worktree", side_effect=Exception("git fail"))
        result = process_work_item(_item(), interactive=False)
        assert result.success is False
...
class TestProcessWorkItemCopilotSuccessGateDisabled:
    ...
    def test_success_gate_disabled(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.create_worktree", return_value=Path("/tmp/wt"))
        monkeypatch.setattr("pokepoke.workflow.invoke_copilot", return_value=_ok_copilot())
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(True, 1))
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=True, request_count=1), True),
        )
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", lambda r: None)
    ...
    def test_success_with_stats_accumulation(self, monkeypatch):
        ...
        monkeypatch.setattr("pokepoke.workflow.create_worktree", return_value=Path("/tmp/wt"))
        monkeypatch.setattr("pokepoke.workflow.invoke_copilot", return_value=_ok_copilot(stats=stats))
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(True, 0))
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", return_value=stats)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
...
class TestProcessWorkItemCopilotFailureNoRetry:
    ...
    def test_copilot_fail_no_retry(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.create_worktree", return_value=Path("/tmp/wt"))
        monkeypatch.setattr("pokepoke.workflow.invoke_copilot", return_value=_fail_copilot())
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", lambda r: None)
        monkeypatch.setattr("pokepoke.workflow._maybe_retry_copilot", return_value=(False, ""))
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
...
class TestProcessWorkItemShutdownDuringLoop:
    ...
    def test_shutdown_exits_loop(self, monkeypatch):
        monkeypatch.setattr("pokepoke.workflow.create_worktree", return_value=Path("/tmp/wt"))
        monkeypatch.setattr("pokepoke.workflow.is_shutting_down", lambda: True)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
...
class TestProcessWorkItemTimeoutRestart:
    ...
    def test_timeout_max_restarts_exceeded(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.time.time", fake_time)
    ...
    def test_timeout_restarts_then_succeeds(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.time.time", lambda: next(times, 1.0))
...
class TestProcessWorkItemCopilotTimeoutWithResume:
    ...
    def test_timeout_then_resume_then_success(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.invoke_copilot", fake_invoke)
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", lambda r: None)
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(True, 0))
        ...
        def fake_maybe_retry(*a, **kw):
            ...
            return (False, "")
        monkeypatch.setattr("pokepoke.workflow._maybe_retry_copilot", fake_maybe_retry)
        monkeypatch.setattr("pokepoke.workflow._apply_gate_feedback", lambda fb, acc, it: (acc + [fb], it + 1))
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
...
class TestProcessWorkItemCleanupFailure:
    ...
    def test_cleanup_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", lambda r: None)
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(False, 1))
        monkeypatch.setattr("pokepoke.workflow._log_failure", lambda *a, **kw: None)
        result = process_work_item(_item(), interactive=False)
        assert result.success is False
...
class TestProcessWorkItemBeadsItemAlreadyClosed:
    ...
    def test_already_closed(self, monkeypatch):
        ...
        monkeypatch.setattr("pokepoke.workflow._extract_agent_stats", lambda r: None)
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.reconciliation.is_beads_item_closed", lambda *a: True)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=True, request_count=1), True),
        )
...
class TestProcessWorkItemGateAgentEnabled:
    ...
    def test_gate_passes(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.run_gate_agent", return_value=gate_ok)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=True, request_count=1), True),
        )
    ...
    def test_gate_rejects_then_work_agent_retries(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.run_gate_agent", fake_gate)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=True, request_count=2), True),
        )
    ...
    def test_gate_crashes_retries_then_raises(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(True, 0))
        monkeypatch.setattr("pokepoke.workflow.run_gate_agent", side_effect=RuntimeError("infra crash"))
    ...
    def test_gate_timeout_retries_then_gives_up(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.run_gate_agent", return_value=gate_timeout)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=False, request_count=1), True),
        )
    ...
    def test_gate_crashed_flag_retries(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow.run_gate_agent", return_value=gate_crash)
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
            lambda *a, **kw: (WorkItemResult(success=False, request_count=1), True),
        )
...
class TestProcessWorkItemCopilotRetryOnFailure:
    ...
    def test_non_timeout_failure_clears_resume_state(self, monkeypatch, _mock_workflow_deps):
        ...
        def fake_retry(*a, **kw):
            ...
            return (False, "")
        monkeypatch.setattr("pokepoke.workflow._maybe_retry_copilot", fake_retry)
        monkeypatch.setattr("pokepoke.workflow._apply_gate_feedback", lambda fb, acc, it: (acc + [fb], it + 1))
        monkeypatch.setattr(
            "pokepoke.workflow._finalize_item_result",
...
class TestProcessWorkItemFeedbackPaths:
    ...
    def test_timeout_feedback_prints_resuming(self, monkeypatch, _mock_workflow_deps):
        ...
        monkeypatch.setattr("pokepoke.workflow._log_commit_status", lambda *a: None)
        monkeypatch.setattr("pokepoke.workflow.run_cleanup_with_timeout", return_value=(True, 0))
        monkeypatch.setattr("pokepoke.workflow._maybe_retry_copilot", lambda *a, **kw: (True, "timeout fb"))
        monkeypatch.setattr("pokepoke.workflow._apply_gate_feedback", lambda fb, acc, it: (acc + [fb], it + 1))

        # After retry succeeds, mock finalize
        def fake_finalize(*a, **kw):
            return (WorkItemResult(success=True, request_count=2), True)
        monkeypatch.setattr("pokepoke.workflow._finalize_item_result", fake_finalize)
        ...
        def retry_once(*a, **kw):
            ...
            if retry_n["n"] == 1:
                return (True, "timeout feedback")
            return (False, "")
        monkeypatch.setattr("pokepoke.workflow._maybe_retry_copilot", retry_once)
