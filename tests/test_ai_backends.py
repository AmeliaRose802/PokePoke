...
@patch("pokepoke.ai_backends.shutil.which", return_value=None)
@patch("pokepoke.ai_backends.get_config")
...
@patch("pokepoke.ai_backends.invoke_copilot_sdk_sync")
...
@patch("pokepoke.ai_backends.subprocess.run")
@patch("pokepoke.ai_backends.shutil.which", return_value="/usr/bin/claude")
...
@patch("pokepoke.ai_backends.subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="claude", timeout=10))
@patch("pokepoke.ai_backends.shutil.which", return_value="/usr/bin/claude")
...
@patch("pokepoke.ai_backends.subprocess.run", side_effect=FileNotFoundError("not found"))
@patch("pokepoke.ai_backends.shutil.which", return_value="/usr/bin/claude")
...
@patch("pokepoke.ai_backends.build_prompt_from_work_item", return_value="auto prompt")
@patch("pokepoke.ai_backends.shutil.which", return_value=None)
